...
Writes validation/backburner_trim.json
"""
...
OUT = os.path.join("validation", "backburner_trim.json")
WAYS = {"the page now": None,
        "a quarter more at an hourly close with RSI 60+": ("rsic60", 0.25),
        "a quarter more at RSI 65+": ("rsic65", 0.25),
        "a quarter more at RSI 70+": ("rsic70", 0.25),
        "a quarter more 3 normal bars over the buy": ("up3", 0.25),
        "a quarter more 5 normal bars over the buy": ("up5", 0.25)}
...
def _work(args):
    ...
    try:
        for w_i, (w, cool) in enumerate(WAYS.items()):
            v_ = dict(PB.STOP, **(PB.CRYPTO_BUYS if kind == "crypto" else {}), trim=cool)
            for r in PB.trades_for(sym, kind, v_):
                rows.append([w_i, float(zlib.crc32(("%s|%s" % (kind, sym)).encode()) * 1000000 + r["k"]), r["pct"], r["risk_pct"],
                             ...
                             r["end"] - r["k"], 1.0 if False else 0.0,
...
def main():
    ...
    print("\n  HIS HLT RULE: SELL MORE AFTER A BIG BOUNCE   %d names  (%.0fs)\n" % (len(names), time.time() - t0))
