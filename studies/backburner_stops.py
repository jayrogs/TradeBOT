"""backburner_stops.py -- eight ways to place the stop on the SAME backburner trades (2026-09-22, his ask after round 2:
...
OUT = os.path.join("validation", "backburner_stops.json")
WAYS = list(PB.STOPS)
...
def _work(args):
    ...
    try:
        for w_i, w in enumerate(WAYS):
            for r in PB.trades_for(sym, kind, PB.STOPS[w]):
...
def main():
    ...
    print("\n  EIGHT STOPS ON THE SAME BACKBURNER TRADES   %d names  (%.0fs)\n" % (len(names), time.time() - t0))
    for cut, sel in (("everything", lambda g: g.pct == g.pct), ("stocks + ETFs", lambda g: g.crypto == 0),
        ...
        print("    %-66s %5s %7s %7s %4s %6s %7s %6s %5s %5s %5s %6s %7s" % (
            "the stop", "n", "avg", "middle", "won", "R", "worst", "5%", "early", "half", "high", "yrs", "blocks"))
        for w_i, w in enumerate(WAYS):
