"""backburner_cool.py -- HIS BURL RULE: a bounce that cools the RSI ends the setup (2026-09-22).
...
Writes validation/backburner_cool.json
"""
...
OUT = os.path.join("validation", "backburner_cool.json")
WAYS = {"the page now (no cooling rule)": None,
        "BURL'S LEVEL: back over 31, then out on the next close under 30": (31, "exit"),
        "BURL'S LEVEL: back over 33, then out on the next close under 30": (33, "exit"),
        "back over 31, then stop under the low of the drop": (31, "low"),
        "back over 33, then stop under the low of the drop": (33, "low"),
        "cooled to 35, then stop under the low of the drop": (35, "low"),
        "cooled to 40, then stop under the low of the drop": (40, "low"),
        "cooled to 35, then out on the next close under 30": (35, "exit"),
        "cooled to 40, then out on the next close under 30": (40, "exit"),
        "READING 2: no second buy once RSI has closed over 35": "c35",
        "READING 2: no second buy after 35, AND stop under the low": "c35low"}
...
def _work(args):
    ...
    try:
        for w_i, (w, cool) in enumerate(WAYS.items()):
            if cool == "c35":
                v_ = dict(PB.STOP, cancel_second=35)
            elif cool == "c35low":
                v_ = dict(PB.STOP, cancel_second=35, cool=(35, "low"))
            else:
                v_ = dict(PB.STOP, cool=cool)
            for r in PB.trades_for(sym, kind, v_):
                rows.append([w_i, float(zlib.crc32(("%s|%s" % (kind, sym)).encode()) * 1000000 + r["k"]), r["pct"], r["risk_pct"],
                             ...
                             r["end"] - r["k"], 1.0 if r["how"].startswith("out: the bounce cooled") else 0.0,
...
def main():
    ...
    print("\n  HIS BURL RULE: A BOUNCE THAT COOLS THE RSI ENDS THE SETUP   %d names  (%.0fs)\n" % (len(names), time.time() - t0))
