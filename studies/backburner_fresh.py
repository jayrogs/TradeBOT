...
OUT = os.path.join("validation", "backburner_fresh.json")
WAYS = ["oscillation: the run did NOT clear its own 60-day high (under 1 normal bar)",
        "borderline (1 to 2 normal bars over it)",
        "HIS BACKBURNER: the run cleared it by 2+ normal bars",
        "cleared it by 4+",
        "everything (what the page does now)"]
CUTS = [lambda f: f < 1, lambda f: (f >= 1) & (f < 2), lambda f: f >= 2, lambda f: f >= 4, lambda f: f == f]
...
def _work(args):
    ...
    try:
        ...
        for w_i, sel in enumerate(CUTS):
            for r in got:
                if r.get("fresh") is None or not bool(sel(np.array([r["fresh"]]))[0]):
                    continue
...
def main():
    ...
    print("\n  HIS RULE: DID THE RUN REACH LEVELS PRICE WAS NOT ALREADY AT   %d names  (%.0fs)\n" % (len(names), time.time() - t0))
