...
SLOTS = [3, 5, 10, 20, 40]
RISK = 0.01                       # at most 1% of the running account at risk per trade
RUNS = 60
# NAMES WITH A ONE-DAY CLOSE-TO-CLOSE MOVE OVER 60% ANYWHERE IN THEIR HISTORY are left out (2026-09-15 review).
# Some are real (ARGX, PCG, RKT), but APLD at exactly 2.000x, QXO at 4.9x, KDP at 0.18x, OVV at 0.28x and SOXS at
# 0.054x are split and reorganisation glitches in the data, and QXO's was carrying a +107% "trade". All 23 out
# together cost 4.3% of the profit; keeping the honest ones is not worth keeping the fakes.
...
def wallet(trades, slots, seed, risk=RISK):
    """Trades in time order into a CASH account with a fixed number of slots. A slot is tied up until its trade
    closes. Each position is sized in dollars at entry: at most `risk` of the account at the stop, at most an
    equal share of the account (1/slots), and never more than the cash actually sitting free. Nothing is
    borrowed.

    THE FIRST VERSION OF THIS WAS 2.2x LEVERAGED (2026-09-15 review): a 1% risk on a 3.9% stop is a 26% position,
    ten of those is 260% of the account, and the cap was 3/slots. Median exposure was 218%, peak 288%. That
    margin was never stated, so it is gone; +40% a year was the levered number."""
    rng = np.random.default_rng(seed)
    eq = 1.0
    cash = 1.0
    open_ = []                      # (exit_date, pct, dollars)
    peak, dip = 1.0, 0.0
    taken = missed = 0
    by_day = {}
    for tr in trades:
    ...
    for day in sorted(by_day):
        while open_ and open_[0][0] <= day:
            _d, pct, dollars = open_.pop(0)
            eq += dollars * pct / 100.0
            cash += dollars * (1.0 + pct / 100.0)
            peak = max(peak, eq)
        ...
        if len(todays) > free:
            ...
            picks = todays
        for tr in picks:
            want = min(risk * eq / (tr["risk_pct"] / 100.0),     # the stop risks at most 1% of the account
                       eq / max(1, slots))                          # and no position is bigger than its share
            dollars = min(want, cash)                               # and you cannot spend what you do not have
            if dollars <= 0:
                missed += 1
                continue
            cash -= dollars
            open_.append((tr["t_out"], tr["pct"], dollars))
            taken += 1
        open_.sort(key=lambda x: x[0])
        curve.append((day, eq))
        expo.append(sum(d_ for _, _, d_ in open_) / eq)
    for _d, pct, dollars in open_:
        eq += dollars * pct / 100.0
        peak = max(peak, eq)
        dip = min(dip, eq / peak - 1.0)
    return eq, dip, taken, missed, curve, (float(np.median(expo)) if expo else 0.0)
...
def main():
    ...
    names = [(s_, k_) for s_, k_ in B.universe() if k_ in ("stock", "etf") and s_ not in SUSPECT]
    print("  %d names (%d left out for suspect data)" % (len(names), len(SUSPECT)))
    R.quiet_workers()
    trades, errs = [], []
    ...
    out = dict(meta=dict(trades=len(trades), years=round(yrs, 1), per_year=round(len(trades) / yrs),
               ...
               wallets={})
    def table(trs, label, slot_list, key):
        print("\n  %s: %d trades, %.0f a year, avg R %+.2f" % (label, len(trs), len(trs) / yrs,
                                                             np.mean([t["R"] for t in trs]) if trs else 0))
        print("  %-8s %12s %12s %12s %10s %10s %10s" % (
            "slots", "a year", "10-90% band", "worst dip", "deployed", "taken", "missed"))
        for slots in slot_list:
            res = [wallet(trs, slots, s) for s in range(RUNS)]
            ...
            ex = np.mean([r[5] for r in res])
            print("  %-8d %11.1f%% %11s %11.1f%% %9.0f%% %10.0f %10.0f" % (
                slots, 100 * ann.mean(),
                "%.0f to %.0f%%" % (100 * np.percentile(ann, 10), 100 * np.percentile(ann, 90)),
                100 * dips.mean(), 100 * ex, tk, ms))
            out["wallets"]["%s%d" % (key, slots)] = dict(
                a_year=float(ann.mean()), lo=float(np.percentile(ann, 10)), hi=float(np.percentile(ann, 90)),
                worst_dip=float(dips.mean()), deployed=float(ex), taken=float(tk), missed=float(ms), n=len(trs))
    table(trades, "EVERY STOCK AND ETF, cash account, no leverage", SLOTS, "")
    # SURVIVORSHIP, bounded two ways. Sector and index ETFs do not go to zero and are rarely delisted, so an
    # ETF-only wallet is the version this bias cannot flatter much. And names whose history starts before the
    # window removes anything that joined the list later (it does NOT bring back anything that left).
    etfs = [t for t in trades if t["kind"] == "etf"]
    table(etfs, "ETFs ONLY (the survivorship-resistant version)", (3, 5, 10), "etf_")
    first_seen = {}
    for t in trades:
        first_seen[t["sym"]] = min(first_seen.get(t["sym"], t["t_in"]), t["t_in"])
    old = [t for t in trades if first_seen[t["sym"]] < "2018-01-01"]
    table(old, "NAMES WITH A TRADE BEFORE 2018 (nothing that joined the list later)", (5, 10), "old_")
    weak = [t for t in trades if 0 <= t["breadth"] < 0.40]
    table(weak, "ONLY WHEN THE MARKET IS WEAK (breadth under 40%)", (3, 5, 10), "weak_")
    out["trades"] = trades
