"""backburner_firstsell.py -- sixteen places to sell THE HALF (the first piece); the rest walked, Dan's stop on the SAME backburner trades (2026-09-22, his ask after round 2:
...
OUT = os.path.join("validation", "backburner_firstsell.json")
WAYS = list(PB.FIRST_SELLS)
...
def _work(args):
    ...
    try:
        for w_i, w in enumerate(WAYS):
            for r in PB.trades_for(sym, kind, dict(PB.STOP, first_sell=PB.FIRST_SELLS[w])):
...
def main():
    ...
    print("\n  SIXTEEN PLACES TO SELL THE HALF, SAME TRADES, DAN'S STOP, THE REST WALKED   %d names  (%.0fs)\n" % (len(names), time.time() - t0))
    for cut, sel in (("everything", lambda g: g.pct == g.pct), ("stocks + ETFs", lambda g: g.crypto == 0),
        ...
        print("    %-66s %5s %7s %7s %4s %6s %7s %6s %5s %5s %5s %6s %7s" % (
            "the half sells at", "n", "avg", "middle", "won", "R", "worst", "5%", "early", "half", "high", "yrs", "blocks"))
        for w_i, w in enumerate(WAYS):
