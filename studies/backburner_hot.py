"""backburner_fresh.py -- his "not a significant run up, just oscillations" rule, on the whole pool on the SAME backburner trades (2026-09-22, his ask after round 2:
...
OUT = os.path.join("validation", "backburner_hot.json")
WAYS = ["the 12 EMA caught up: price UNDER it at the dip by half a bar or more",
        "just under it (0 to half a bar)",
        "STILL HOT: price still above the daily 12 EMA at the dip",
        "well above it (half a bar or more)",
        "everything (what the page does now)"]
CUTS = [lambda f: f < -0.5, lambda f: (f >= -0.5) & (f < 0), lambda f: f >= 0, lambda f: f >= 0.5, lambda f: f == f]
...
def _work(args):
    ...
    try:
        ...
        for w_i, sel in enumerate(CUTS):
            for r in got:
                if r.get("hot") is None or not bool(sel(np.array([r["hot"]]))[0]):
                    continue
...
def main():
    ...
    print("\n  HIS RULE: IS THE NAME STILL RUNNING HOT AT THE DIP   %d names  (%.0fs)\n" % (len(names), time.time() - t0))
