...
Writes validation/backburner_gate.json
"""
...
OUT = os.path.join("validation", "backburner_gate.json")
WAYS = {"OLD: the stop decided from this hour's own close": False,
        "HONEST: the stop decided from the last close": True}
...
def _work(args):
    ...
    try:
        for w_i, (w, cool) in enumerate(WAYS.items()):
            v_ = dict(PB.STOP, **(PB.CRYPTO_BUYS if kind == "crypto" else {}), gate_prev=cool)
...
def main():
    ...
    print("\n  WHICH STOP IS RESTING: DECIDED FROM THIS CLOSE (OLD) OR THE LAST ONE (HONEST)   %d names  (%.0fs)\n" % (len(names), time.time() - t0))
