"""trend_death.py -- the trend ride in the owner's exact words (2026-09-06):
"catch a higher low and ride... if the trend dies, you get out."

  entry   a higher low confirms WHILE the chart is in an uptrend (the live
          trend grammar in structure.py: a higher low and a higher high have
          both printed and nothing has killed it). Buy the next open. Skip it
          if that open is already more than one normal bar's move above the
          pivot.
  exit    the uptrend dies: a wick under the last higher low (beyond the
          same-level tolerance) or a lower high / lower low printing. Sell at
          the next open. No room given -- that was a backburner idea, not a
          trend-riding one ("the 3 ATR thing was mainly for backburners").
  compared with the loose trailing stop from trend_trail.py on the same entries.
  partial none, or a third at twice the risk.
...
OUT = os.path.join("validation", "trend_death.json")
BARS_DAY_TF = 24

TAKES = [("no partial", None, 0.0), ("a third at 2R", "2R", 1 / 3)]
RUNS = ["trend dies", "higher low", "trail 8 bars under the high"]
HOLD_DAYS = {}
TRAIL = {"trail 8 bars under the high": 8.0}
...
def run_exit(c, o, ema, a14, lows, lcis, highs, hcis, e, level, n, cap, mode, hi_stop):
    ...
    if mode == "trend dies":
        # s1 is the LAST bar of the uptrend. The bar after it (s1+1) is the one
        # whose close revealed the death (a contrary pivot confirms on its close,
        # a wick break happens on it), so the earliest honest sale is the open
        # of s1+2. Selling at o[s1+1] was a one-bar look-ahead (caught 2026-09-06
        # on JPM 5m: sold at the open of the bar the lower high confirmed on).
        s1 = int(hi_stop[e]) if np.isfinite(hi_stop[e]) else -1
        if s1 < e or s1 + 2 >= n:
            return None
        for k in range(e, s1 + 2):
            worst = min(worst, c[k] / fill - 1)
        return s1 + 1, o[s1 + 2], "trend died", worst
...
def study_frame(sym, kind, tf, frames, start):
    ...
    hi = np.full(n, np.nan)                     # here: the bar the current uptrend ends on
    for kind_, s0, s1 in ST.spans(df, causal=True):
        if kind_ == "UP":
            hi[s0:s1 + 1] = s1
    in_up = np.isfinite(hi)
    age = np.zeros(n, dtype=int)
    for kind_, s0, s1 in ST.spans(df, causal=True):
        if kind_ == "UP":
            age[s0:s1 + 1] = np.arange(s1 - s0 + 1)
    ci_ = B.CHAIN.index(tf)
    up1 = frames.get(B.CHAIN[ci_ + 1]) if ci_ + 1 < len(B.CHAIN) else None
    t1 = B.higher_view(df, tf, up1, B.CHAIN[ci_ + 1])[0] if up1 is not None else None
    ...
    for ci, j, price, lab in lows:
        if lab != "HL" or ci < 30 or ci + 2 >= n - 1:
            continue
        if not in_up[ci]:
            continue                                   # the trend has to be alive
        e = ci + 1
        ...
        hi_up = ("higher tf up" if t1 is not None and str(t1[ci]) == "UP" else
                 "higher tf down" if t1 is not None and str(t1[ci]) == "DOWN" else "higher tf flat")
        trend_age = ("uptrend just opened (under 3 bars)" if age[ci] < 3 else
                     "uptrend 3-10 bars old" if age[ci] < 10 else "uptrend 10+ bars old")
        ...
        for take_label, trig, size in TAKES:
            ...
            for mode in RUNS:
                ...
                rows.append(dict(sym=sym, kind=kind, tf=tf, t=str(t), era=era, null=0,
                                 hi_up=hi_up, trend_age=trend_age,
...
KEYS = ["variant", "take", "run", "tf", "era", "kind", "hi_up", "trend_age"]
...
def main():
    ...
    res = {"by_variant": by(folded, ["variant"]),
           ...
           "by_run_hiup": by(folded, ["run", "hi_up"]),
           "by_run_age": by(folded, ["run", "trend_age"]),
           "by_run_hiup_tf": by(folded, ["run", "hi_up", "tf"]),
           "by_run_age_tf": by(folded, ["run", "trend_age", "tf"]),
           "by_run_both": by(folded, ["run", "hi_up", "trend_age"]),
