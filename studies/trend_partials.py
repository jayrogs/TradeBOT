"""trend_partials.py -- sell a part, let the rest run as long as it can.
...
OUT = os.path.join("validation", "trend_partials.json")

TAKES = [("no partial", None, 0.0), ("a third at 1R", "1R", 1 / 3), ("half at 1R", "1R", 0.5),
         ("a third at 2R", "2R", 1 / 3), ("half at 2R", "2R", 0.5), ("half at a new high", "hh", 0.5)]
RUNS = ["higher low", "one behind", "clear break", "12 EMA", "higher timeframe", "nothing (30-day cap)"]


def run_exit(c, o, ema, a14, lows, lcis, highs, hcis, e, level, n, cap, mode, hi_stop):
    """Where the runner gets out. Returns (bar, price, why)."""
    p = bisect.bisect_right(lcis, e - 1)
    pending = None
    for k in range(e, n - 1):
        if mode in ("higher low", "one behind", "clear break"):
            ...
            buf = BUF * a14[k] if np.isfinite(a14[k]) else 0.0
            if c[k] < ema[k] - buf:
                return k, o[k + 1], "under the 12 EMA"
        elif mode == "higher timeframe":
            if np.isfinite(hi_stop[k]) and c[k] < hi_stop[k]:
                return k, o[k + 1], "higher timeframe broke"
        if k - e >= cap:
            return k, o[k + 1], "cap"
    return None
...
def fold(rows):
    ...
    out = g.agg(n=("ret", "size"), sum_ret=("ret", "sum"), sum_drift=("drift", "sum"),
                ...
                sum_held=("held", "sum"), sum_left=("left", "sum"),
...
def main():
    ...
    print()
    print("  %-52s %9s %9s %9s %6s %6s %7s %10s" % ("variant", "n", "per trade", "vs drift", "win", "took", "held", "left after"))
    for v, b in sorted(res["by_variant"].items(), key=lambda kv: -kv[1]["ret"]):
        print("  %-52s %9d %+8.2f%% %+8.2f%% %5.0f%% %5.0f%% %6.0f %+9.2f%%" % (
            v, b["n"], 100 * b["ret"], 100 * b["edge"], 100 * b["win"], 100 * b["took"], b["held"], 100 * b["left"]))
    print()
    print("  by how long the runner is held:")
    for v, b in sorted(res["by_run"].items(), key=lambda kv: -kv[1]["ret"]):
        print("    %-24s n=%9d %+7.2f%%  held %6.0f bars  left after %+.2f%%" % (v, b["n"], 100 * b["ret"], b["held"], 100 * b["left"]))
    print()
